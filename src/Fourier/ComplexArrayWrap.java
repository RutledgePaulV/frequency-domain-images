...
public class ComplexArrayWrap
{
	...
	public ComplexArrayWrap(double[][] pixels) 
	{
		...
		for(int row = 0; row < size; row++)
			for(int col = 0; col < size; col++)
				values[row][col] = (row<height && col<width)?new Complex(Math.pow(-1, row+col)*pixels[row][col],0):new Complex(0,0);
	}
	...
	public void Convolve(Kernel k)
	{
		...
		for(int row = 0; row < size; row++)
			for(int col = 0; col < size; col++)
					values[row][col] = values[row][col].Multiply(new Complex(k.GetFactor(2*(col*ratio)-1, 2*(row*ratio)-1),0));
	}
